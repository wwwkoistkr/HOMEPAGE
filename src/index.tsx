...
import type { Bindings, Variables, Department, Popup, Notice, ProgressItem, DepPage, FAQ } from './types';
...
// ===== Admin API Routes =====
// Login & change-password don't need auth middleware (handled inside admin router)
...
// About pages (redirect to appropriate service)
app.get('/about/:page', async (c) => {
  ...
  const info = aboutContent[page];
  if (!info) return c.redirect('/');
  ...
  const contentHtml = `
    <section class="bg-gradient-to-r from-primary to-primary-light py-[clamp(2.5rem,5vh,4rem)]">
      <div class="w-[min(92vw,1400px)] mx-auto px-[clamp(1rem,3vw,3rem)]">
        <h1 class="text-white font-bold text-[clamp(1.4rem,3vw,2.2rem)]"><i class="fas fa-building mr-2"></i>${info.title}</h1>
      </div>
    </section>
    <section class="py-[clamp(2rem,4vh,4rem)]">
      <div class="w-[min(92vw,1000px)] mx-auto px-[clamp(1rem,3vw,3rem)]">
        <div class="bg-white rounded-xl border border-gray-100 p-[clamp(1.5rem,3vw,3rem)] prose prose-gray max-w-none">${info.content}</div>
      </div>
    </section>`;

  return c.html(layout({ settings, departments, title: info.title, content: contentHtml }));
});
...
// Admin CRUD pages
const adminPages = ['site-settings', 'departments', 'popups', 'notices', 'progress', 'downloads', 'faqs', 'inquiries'];
