    ...
    <div class="fluid-container" style="max-width:min(1100px, 100% - var(--container-pad) * 2)">
      <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead><tr class="bg-primary text-slate-300 f-text-xs">
              <th class="text-left font-medium" style="padding:var(--space-sm) var(--space-md)">구분</th>
              <th class="text-left font-medium" style="padding:var(--space-sm) var(--space-md)">제품명</th>
              <th class="text-left font-medium hidden sm:table-cell" style="padding:var(--space-sm) var(--space-md)">업체</th>
              <th class="text-center font-medium" style="padding:var(--space-sm) var(--space-md)">상태</th>
              <th class="text-center font-medium hidden sm:table-cell" style="padding:var(--space-sm) var(--space-md)">시작일</th>
            </tr></thead>
            <tbody>
              ${items.map(p => `
              <tr class="border-t border-slate-100 hover:bg-blue-50/30 transition-colors">
                <td class="text-slate-500 f-text-xs" style="padding:var(--space-sm) var(--space-md)">${p.category}</td>
                <td class="font-medium text-slate-800 f-text-xs" style="padding:var(--space-sm) var(--space-md)">${p.product_name}</td>
                <td class="text-slate-600 hidden sm:table-cell f-text-xs" style="padding:var(--space-sm) var(--space-md)">${p.company || '-'}</td>
                <td class="text-center" style="padding:var(--space-sm) var(--space-md)"><span class="inline-flex items-center gap-1 rounded-full font-medium f-text-xs ${p.status === '완료' ? 'bg-green-50 text-green-600' : 'bg-amber-50 text-amber-600'}" style="padding:2px var(--space-sm)">${p.status}</span></td>
                <td class="text-center text-slate-400 hidden sm:table-cell f-text-xs" style="padding:var(--space-sm) var(--space-md)">${p.start_date || '-'}</td>
              </tr>
              `).join('')}
              ${items.length === 0 ? '<tr><td colspan="5" class="text-center text-slate-400 f-text-sm" style="padding:var(--space-2xl) 0">등록된 현황이 없습니다.</td></tr>' : ''}
            </tbody>
          </table>
        </div>
      </div>
