...
export function homePage(opts: {
  ...
  <!-- Hero Section - Soft Professional Style -->
  <section class="relative min-h-[clamp(420px,60vh,700px)] bg-gradient-to-br from-[#F0F4F8] via-[#F5F6FA] to-[#EBF0F7] flex items-center overflow-hidden">
    <!-- Background Pattern - Soft -->
    <div class="absolute inset-0">
      <div class="absolute top-0 right-0 w-[50%] h-full bg-gradient-to-l from-slate-200/30 to-transparent"></div>
      <div class="absolute bottom-0 left-0 w-full h-1/3 bg-gradient-to-t from-slate-100/40 to-transparent"></div>
      <svg class="absolute bottom-0 left-0 w-full opacity-20" viewBox="0 0 1440 320" fill="none">
        <path d="M0,160L48,176C96,192,192,224,288,213.3C384,203,480,149,576,138.7C672,128,768,160,864,181.3C960,203,1056,213,1152,202.7C1248,192,1344,160,1392,144L1440,128V320H0Z" fill="#CBD5E1"/>
      </svg>
      <!-- Floating decorative elements -->
      <div class="absolute top-[15%] left-[8%] text-slate-300/20 text-6xl animate-pulse"><i class="fas fa-shield-halved"></i></div>
      <div class="absolute top-[35%] right-[12%] text-slate-300/15 text-5xl animate-pulse" style="animation-delay:1s"><i class="fas fa-lock"></i></div>
      <div class="absolute bottom-[25%] left-[22%] text-slate-300/15 text-4xl animate-pulse" style="animation-delay:2s"><i class="fas fa-key"></i></div>
    </div>
    ...
    <div class="relative w-[min(92vw,1400px)] mx-auto px-[clamp(1rem,3vw,3rem)]">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-[clamp(2rem,4vw,4rem)] items-center">
        ...
        <div data-aos="fade-right">
          <div class="inline-flex items-center gap-2 bg-accent/8 border border-accent/15 px-4 py-2 rounded-full mb-6">
            <span class="w-2 h-2 bg-accent rounded-full animate-pulse"></span>
            <span class="text-accent font-medium text-[clamp(0.7rem,1vw,0.85rem)]">국가 공인 정보보안 시험·평가 전문기관</span>
          </div>
          <h1 class="text-[#1a2332] font-black text-[clamp(1.1rem,2.2vw,1.9rem)] leading-[1.3] mb-6">
            ${s.site_slogan || '최상의 시험·인증 서비스로<br><span class="text-accent">정보보안 기술</span>을 완성'}
          </h1>
          <p class="text-[#64748B] text-[clamp(0.7rem,0.8vw,0.85rem)] leading-relaxed mb-8 max-w-lg">
        ...
        <div data-aos="fade-left" data-aos-delay="200" class="flex justify-center lg:justify-end">
          <div class="relative bg-[#FAFBFC] rounded-2xl shadow-lg shadow-slate-200/40 border border-slate-200/60 p-[clamp(1.5rem,3vw,2.5rem)] max-w-md w-full">
            <!-- Accent top border -->
            ...
            <div class="text-[#64748B] text-[clamp(0.7rem,1vw,0.9rem)] font-medium mb-2 tracking-wider uppercase">
              <i class="fas fa-headset mr-1 text-accent"></i> 상담문의
            </div>
            
            <a href="tel:${s.phone || '02-586-1230'}" 
               class="block text-[#1a2332] font-black text-[clamp(1.2rem,2.5vw,2rem)] leading-tight mb-4 hover:text-accent transition-colors tracking-tight">
              ${s.phone_display || s.phone || '02-586-1230'}
            </a>
            
            <div class="space-y-2.5 text-[#4A5568]">
              <div class="flex items-center gap-2.5 text-[clamp(0.75rem,1.1vw,1rem)]">
                <div class="w-7 h-7 bg-slate-100 rounded-lg flex items-center justify-center shrink-0"><i class="fas fa-envelope text-accent text-xs"></i></div>
                <a href="mailto:${s.email || 'koist@koist.kr'}" class="hover:text-accent transition-colors">${s.email || 'koist@koist.kr'}</a>
              </div>
              <div class="flex items-center gap-2.5 text-[clamp(0.75rem,1.1vw,1rem)]">
                <div class="w-7 h-7 bg-slate-100 rounded-lg flex items-center justify-center shrink-0"><i class="fas fa-fax text-accent text-xs"></i></div>
                <span>FAX ${s.fax || '02-586-1238'}</span>
              </div>
              <div class="flex items-start gap-2.5 text-[clamp(0.7rem,1vw,0.9rem)]">
                <div class="w-7 h-7 bg-slate-100 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><i class="fas fa-location-dot text-accent text-xs"></i></div>
  ...
  <!-- Services Section -->
  <section id="services" class="py-[clamp(3rem,6vh,6rem)] bg-gradient-to-b from-[#F5F6FA] to-[#EFF2F7]">
    <div class="w-[min(92vw,1400px)] mx-auto px-[clamp(1rem,3vw,3rem)]">
      <div class="text-center mb-[clamp(2rem,4vh,4rem)]" data-aos="fade-up">
        <h2 class="text-[clamp(1.4rem,3vw,2.5rem)] font-bold text-[#1a2332] mb-3">핵심 사업분야</h2>
        <p class="text-[#64748B] text-[clamp(0.85rem,1.2vw,1.1rem)]">KOIST의 모든 사업 분야를 한눈에 확인하세요</p>
      </div>
      
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-[clamp(0.8rem,1.5vw,1.5rem)]">
        ${deps.map((dept, i) => `
        <a href="/services/${dept.slug}" class="service-card group block bg-[#FAFBFC] border border-slate-200/60 rounded-xl p-[clamp(1.2rem,2vw,1.8rem)] hover:border-transparent relative overflow-hidden" data-aos="fade-up" data-aos-delay="${i * 60}">
          ...
          <h3 class="font-bold text-[#2D3748] text-[clamp(0.9rem,1.1vw,1.1rem)] mb-2 group-hover:text-accent transition-colors">${dept.name}</h3>
  ...
  <section class="py-[clamp(3rem,6vh,6rem)] bg-[#FAFBFC]">
    <div class="w-[min(92vw,1400px)] mx-auto px-[clamp(1rem,3vw,3rem)]">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-[clamp(1.5rem,3vw,3rem)]">
        
        <!-- Notices -->
        <div data-aos="fade-right" class="bg-[#FEFEFE] rounded-2xl p-[clamp(1.5rem,2.5vw,2.5rem)] shadow-sm shadow-slate-200/30 border border-slate-200/60">
          <div class="flex justify-between items-center mb-5">
            <h3 class="text-[clamp(1.1rem,1.5vw,1.4rem)] font-bold text-[#1a2332] flex items-center gap-2">
        ...
        <!-- Progress -->
        <div data-aos="fade-left" class="bg-[#FEFEFE] rounded-2xl p-[clamp(1.5rem,2.5vw,2.5rem)] shadow-sm shadow-slate-200/30 border border-slate-200/60">
          <div class="flex justify-between items-center mb-5">
            <h3 class="text-[clamp(1.1rem,1.5vw,1.4rem)] font-bold text-[#1a2332] flex items-center gap-2">
  ...
  <!-- CTA Section - Soft Professional Style -->
  <section class="py-[clamp(3rem,5vh,5rem)] bg-gradient-to-r from-[#2563EB] to-[#3B82F6]">
    <div class="w-[min(92vw,1400px)] mx-auto px-[clamp(1rem,3vw,3rem)] text-center" data-aos="fade-up">
      ...
      <p class="text-white/85 text-[clamp(0.85rem,1.2vw,1.1rem)] mb-6">전문 상담원이 빠르고 정확하게 안내해 드립니다</p>
